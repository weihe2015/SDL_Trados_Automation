...
namespace StudioIntegrationApiSample
{
    class ContentConnector
    {
        ...
        {
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Studio 2014\\IncomingRequests");
        }

        private static string GetAcceptedRequestsFolder()
        {
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Studio 2014\\AcceptedRequests");
        }
